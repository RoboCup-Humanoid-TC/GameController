...
pub struct HlPenaltyKick {
    pub side: Side,
    pub seconds: i64,
}

impl Action for HlPenaltyKick {
    fn execute(&self, c: &mut ActionContext) {
        if c.game.sec_state.state == SecState::Normal {
            c.game.sec_state.state = SecState::Penaltykick;
            c.game.sec_state.side = self.side;
        } else if c.game.sec_state.state == SecState::Penaltykick && c.game.sec_state.phase == 0 {
        ...
        } else if c.game.sec_state.state == SecState::Penaltykick && c.game.sec_state.phase == 1 {
            c.game.sec_state.phase = 2;
            c.game.secondary_timer = Timer::Stopped;
        } else if c.game.sec_state.state == SecState::Penaltykick && c.game.sec_state.phase == 2 {
    ...
    fn is_legal(&self, c: &ActionContext) -> bool {
        c.game.state == State::Playing && (c.game.sec_state.state == SecState::Normal || (c.game.sec_state.state == SecState::Penaltykick  
            && c.game.sec_state.side == self.side))
    }
}