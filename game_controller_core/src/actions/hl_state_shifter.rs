...
use crate::types::{Phase, Side, State, SecState, SecondaryState, SetPlay, Penalty};
...
impl Action for HlStateShifter {
    ...
    fn is_legal(&self, c: &ActionContext) -> bool {
        if self.state == c.game.state {
            false
        } else if self.state == State::Initial && c.game.phase == Phase::FirstHalf && c.game.state == State::Finished{
            true
